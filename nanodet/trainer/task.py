...
from nanodet.model.loss import distillation_loss

from ..model.arch import build_model
from ..model.loss.distillation_loss import DistillationLoss
...
class TrainingTask(LightningModule):
    ...
    def __init__(self, cfg, evaluator=None):
        ...
        if "distillation" in cfg:
            ...
            self.teacher = build_model(cfg.distillation.teacher_model)

            # 1. Load the Checkpoint
            checkpoint_path = cfg.distillation.teacher_checkpoint
            ckpt = torch.load(checkpoint_path, map_location='cpu')

            # 2. Extract State Dict
            state_dict = ckpt["state_dict"] if "state_dict" in ckpt else ckpt

            # 3. FIX: Strip the "model." prefix generated by Lightning
            new_state_dict = {}
            for k, v in state_dict.items():
                # Remove "model." if it exists at the start
                if k.startswith("model."):
                    name = k[6:]  # Slice off first 6 chars
                else:
                    name = k
                new_state_dict[name] = v

            # 4. Load Weights (strict=False ignores minor AuxHead mismatches)
            ...
            # 6. Init Loss
            self.distill_loss_func = DistillationLoss(feat_weight=2.0)
            print("=> Teacher Loaded, Prefix Stripped, and Frozen.")
        else:
            self.teacher = None
    ...
    def training_step(self, batch, batch_idx):
        ...
        if getattr(self, "teacher", None) is not None:
            # A. Get Teacher Features (Frozen)
            with torch.no_grad():
                self.teacher.eval()
                t_feats = self.teacher.backbone(batch['img'])
                if hasattr(self.teacher, 'fpn'):
                    t_feats = self.teacher.fpn(t_feats)

            # B. Get Student Features (With Gradients)
            # We must re-run backbone. This adds overhead but is the safest "patch" way.
            s_feats = self.model.backbone(batch['img'])
            if hasattr(self.model, 'fpn'):
                s_feats = self.model.fpn(s_feats)

            # C. Calculate Loss
            distill_loss = 0
            import torch.nn.functional as F

            # Zip allows iterating 3 scales (P3, P4, P5)
            for s_map, t_map in zip(s_feats, t_feats):
                # 1. Channel Check
                if s_map.shape[1] != t_map.shape[1]:
                    # If channels differ, we can't distill this layer without a 1x1 conv.
                    print("skipped")
                    continue

                if s_map.shape[2:] != t_map.shape[2:]:
                    t_map = F.interpolate(t_map, size=s_map.shape[2:], mode='nearest')

                distill_loss += F.mse_loss(s_map, t_map)
            loss += (distill_loss * 5.0)
            loss_states["loss_distill"] = distill_loss
